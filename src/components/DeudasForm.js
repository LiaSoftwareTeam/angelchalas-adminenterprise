...
const DeudasForm = ({ deuda, onSubmit, onCancel }) => {
  ...
  useEffect(() => {
    if (deuda) {
      // Si se está editando una deuda existente, llenar el formulario con sus datos
      setFormData({
        proveedor: deuda.proveedor || '',
        descripcion: deuda.descripcion || '',
        fechaEmision: deuda.fechaEmision || '',
        fechaVencimiento: deuda.fechaVencimiento || '',
        monto: deuda.monto ? deuda.monto.toString().replace('$', '').replace(',', '') : '',
  ...
  const handleSubmit = (e) => {
    ...
    const deudaData = {
      ...
      id: deuda ? deuda.id : Date.now() // Usar ID existente o generar uno nuevo
    };
  ...
  return (
    <div className={styles.formContainer}>
      <h3 className={styles.formTitle}>{deuda ? 'Editar Cuenta por Pagar' : 'Registrar Nueva Cuenta por Pagar'}</h3>
      <form onSubmit={handleSubmit}>
        <div className={styles.formGrid}>
          <div className={styles.formGroup}>
            <label className={styles.formLabel} htmlFor="proveedor">Proveedor</label>
            <input
              type="text"
              id="proveedor"
              name="proveedor"
        ...
        <div className={styles.formActions}>
          ...
          <button type="submit" className={styles.submitButton}>
            ...
            {deuda ? 'Actualizar Cuenta' : 'Guardar Cuenta'}
          </button>
        </div>
      </form>
    </div>
